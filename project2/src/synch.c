...
void LockAquire(Lock *lock)
{
  ...
  //Disable interrupts
  intrs = DisableIntrs();
  dbprintf('s', "Proc 0x%x aquiring lock 0x%x.\n", currentPCB, lock);

  if(lock->owner == NULL)
  {
    lock->owner = currentPCB;
  }
  else if (lock->owner != currentPCB)
  {
    l = QueueAllocLink();
    QueueLinkInit (l, (void *)currentPCB);
    dbprintf('s', "Suspending current proc (0x%x).\n", currentPCB);
    QueueInsertLast(&lock->waiting, l);
    ProcessSleep();
  }
...
LockHandleRelease(lock_t lock)
{
  if (lock >=0 && lock < MAX_LOCKS)
  {
    if (locks[lock].inuse == 1 && locks[lock].owner == currentPCB)
    {
      LockRelease(&locks[lock]);
      return 0;
    }
    return 1;
  }
...
void LockRelease(Lock *lock)
{
  ...
  dbprintf('s', "Releasing lock 0x%x.\n", lock);
  lock -> owner = NULL;
  if(!QueueEmpty(&lock->waiting))
  {
    l = QueueFirst(&lock->waiting);
    QueueRemove(l);
    dbprintf('s', "Waking up PCB 0x%x.\n", l->object);
    ProcessWakeup ((PCB *)(l->object));
    QueueFreeLink(l);
