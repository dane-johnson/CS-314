...
// to happen).
static Queue	runQueue[NUM_OF_RUNQUEUE];
static uint32   currentPriority = NUM_OF_RUNQUEUE - 1;
static uint32   starvingCheckCounter = 0;
...
ProcessSchedule ()
{
  PCB           *pcb;
  int           i, j;
  int           liveCounter, iterations;
  Link*         curr;

  dbprintf ('p', "Now entering ProcessSchedule (cur=0x%x, %d priority level)\n",
            currentPCB, currentPriority);

  starvingCheckCounter += processQuantum;
  
  //Increment the wait time for all runnable processes
  for(i = 0; i < NUM_OF_RUNQUEUE; i++)
  {
    if(!QueueEmpty(&runQueue[i]))
    {
      //add one to all processes' wait time
      iterations = QueueLength(&runQueue[i]);
      for(curr = QueueFirst(&runQueue[i]), j = 0; j < iterations; curr = QueueNext(curr), j++)
      {
	pcb = (PCB*)curr->object;
	if(pcb != currentPCB)
	{
	  pcb->waitTime += 1;
	  dbprintf('p', "Incremented wait time for PCB 0x%x, now at %d.\n", pcb, pcb->waitTime);
	  if(pcb->waitTime > STARVING_THRESHOLD)
	  {
	    pcb->scheduleFlags |= PROCESS_STARVING;
	  }
	}
	if(starvingCheckCounter > ONE_SECOND && pcb->scheduleFlags & PROCESS_STARVING)
	{
	  dbprintf('p', "PCB 0x%x is starving, promoting to priority level %d.\n", pcb, STARVING_PRIORITY_LEVEL);
	  QueueRemove(&pcb->l);
	  pcb->currentPriority = STARVING_PRIORITY_LEVEL;
	  pcb->quantum = STARTING_QUANTUM * 2;
	  QueueInsertLast(&runQueue[pcb->currentPriority], &pcb->l);
	  break;
	}  
      }
    }
  ...
  if(starvingCheckCounter > ONE_SECOND)
  {
    starvingCheckCounter = 0;
  }
    
  pcb = NULL;
  //Scan for a process with a higher priority
  dbprintf('p', "Looking for a process with a higher priority\n");
  for(i = NUM_OF_RUNQUEUE - 1; i > currentPriority; i--)
  {
    if(!QueueEmpty(&runQueue[i]))
    {
  ...
  //If we've found a new process, schedule it
  if(pcb != NULL)
  {
    scheduleProcessHelper(pcb);
  }
  else
  {
    //Didn't find a higher priority
    dbprintf('p',"Looking for a job at the current priority.\n")
    //Check to see if there are jobs at the current priority
    if(!QueueEmpty(&runQueue[currentPriority]))
    {
      //Check what the first job is
      pcb = (PCB*)(QueueFirst(&runQueue[currentPriority])->object);
      if (pcb == currentPCB)
      {
	//deduct the penalty
	pcb->quantum -= QUANTUM_DEDUCTION;
	if(pcb->quantum <= 0)
	{
	  //Determine what to do with our expired process
	  if(pcb->scheduleFlags & PROCESS_STARVING)
	  {
	    //The process has been boosted to avoid starvation, remove this flag and return it to base priority
	    pcb->scheduleFlags &= ~PROCESS_STARVING;
	    pcb->currentPriority = pcb->basePriority;
	  }
	  else if (pcb->scheduleFlags & PROCESS_BOOSTED)
	  {
	    //The process has been boosted after falling victim to an IO event, it may be above it's base priority
	    if(pcb->basePriority < pcb->currentPriority)
	    {
	      pcb->basePriority -= 1;
	    }
	    if(pcb->basePriority == pcb->currentPriority)
	    {
	      pcb->scheduleFlags &= ~PROCESS_BOOSTED;
	    }
	  }
	  QueueRemove(&pcb->l);
	  QueueInsertLast(&runQueue[pcb->currentPriority], &pcb->l);
	  //Schedule the next process, if it was the previous one so be it.
	  pcb = (PCB*)(QueueFirst(&runQueue[pcb->currentPriority])->object);
	  scheduleProcessHelper(pcb);
	}
	else
	{
	  dbprintf('p', "Rescheduling previous process 0x%x\n", currentPCB);
	}
      }
      else
      {
	//Found another job at the current priority level
	scheduleProcessHelper(pcb);
      }
    }
    else
    {
      pcb = NULL;
      //Now, check for processes below the priority of the current process
      dbprintf('p', "Looking for processes below the current priority\n");
      for(i = currentPriority - 1; i > 0; i--)
      {
	if(!QueueEmpty(&runQueue[i]))
	{
	  pcb = (PCB*)(QueueFirst(&runQueue[i])->object);
	  dbprintf('p', "Found runnable process 0x%x\n", pcb);
	  break;
	}
      } 
      //If found a process to run
      if(pcb != NULL)
      {
	scheduleProcessHelper(pcb);
      }
	...
	dbprintf('p', "There are %d runqueues with processes available at exit time.\n", liveCounter);
	dbprintf('p', "There are %d suspended processes at exit time.\n", QueueLength(&waitQueue))
  ...
  dbprintf ('p', "Leaving ProcessSchedule (cur=0x%x, quantum=%d)\n", currentPCB, currentPCB->quantum);
}

void scheduleProcessHelper(PCB* pcb)
{
  dbprintf('p', "Scheduling Process 0x%x\n", pcb);
  currentPriority = pcb->currentPriority;
  currentPCB = pcb;
  if(pcb->quantum <= 0)
  {
    pcb->quantum = STARTING_QUANTUM;
  }
}

//----------------------------------------------------------------------
...
ProcessWakeup (PCB *wakeup)
{
  ...
  wakeup->scheduleFlags |= PROCESS_BOOSTED;
  if (wakeup->currentPriority + 2 < STARVING_PRIORITY_LEVEL)
  {
    wakeup->currentPriority += 2;
  }
  else
  {
    wakeup->currentPriority = STARVING_PRIORITY_LEVEL - 1;
  }
  wakeup->quantum -= 1;
  QueueInsertLast (&runQueue[wakeup->currentPriority], &wakeup->l);
...
{
  int oldQueueNum;
  oldQueueNum = pcb->currentPriority;
  dbprintf('p', "Entering ProcessDestroy for 0x%x.\n", pcb);
  ProcessSetStatus (pcb, PROCESS_STATUS_ZOMBIE);
  dbprintf('p', "Removing process from runqueue %d\n", oldQueueNum);
  dbprintf('p', "Before removal, queuelength %d\n", QueueLength(&runQueue[oldQueueNum]));
  QueueRemove (&pcb->l);
  dbprintf('p', "After removal, queuelength %d\n", QueueLength(&runQueue[oldQueueNum]));
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  intrs = DisableIntrs ();
  QueueInsertLast (&runQueue[pcb->currentPriority], l);
  RestoreIntrs (intrs);
